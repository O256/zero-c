...
mov [es:0x1a], word 0x8000 ; Base=0x8000，这是低16位
mov [es:0x1c], byte 0x00 ; 这是Base的高8位
...
;4号段
;基址0x0000,上限0xfffff
mov [es:0x20], word 0x00ff ; Limit=0xffff
mov [es:0x22], word 0x0000 ; Base=0x0000，这是低16位
...
mov [es:0x27], byte 0x00 ; Base的高8位

;5号段, 64位代码段
;基址0x0000,上限0xfffff
mov [es:0x28], word 0x00ff ; Limit=0xffff
mov [es:0x2a], word 0x0000 ; Base=0x0000，这是低16位
mov [es:0x2c], byte 0x00 ; 这是Base的高8位
mov [es:0x2d], byte 1_00_1_101_0b ; P=1, DPL=0, S=1, Type=101b, A=0
mov [es:0x2e], byte 1_0_1_0_0000b  ; G=1, D/B=1, AVL=00, Limit的高4位是0000
mov [es:0x2f], byte 0x00 ; Base的高8位

;6号段, 64位数据段
;基址0x0000,上限0xfffff
mov [es:0x30], word 0x00ff ; Limit=0xffff
mov [es:0x32], word 0x0000 ; Base=0x0000，这是低16位
mov [es:0x34], byte 0x00 ; 这是Base的高8位
mov [es:0x35], byte 1_00_1_001_0b ; P=1, DPL=0, S=1, Type=001b, A=0
mov [es:0x36], byte 1_0_1_0_0000b  ; G=1, D/B=1, AVL=00, Limit的高4位是0000

; 下面是gdt信息的配置
mov ax, 0x07f0
mov es, ax
mov [es:0x00], word 55 ; GDT的大小
